...
### government2behavior
```{r}
lm_local2ac <- lm(action ~ eva_loacl_z + age + female + edu + socailLevel_now + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_cen2ac <- lm(action ~ eva_central_z + age + female + edu + socailLevel_now  + lag(imcomePer)  + party + race + rural + health + marriage,data = cgss2021)

lm_local2media <- lm(mediaNew ~ eva_loacl_z + age + female + edu + socailLevel_now + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_local_med2ac <- lm(action ~ eva_loacl_z + age + female + edu + socailLevel_now + mediaNew + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_cen_med2ac <- lm(action ~ eva_central_z + age + female + edu + socailLevel_now + mediaNew + lag(imcomePer)  + party + race + rural + health + marriage,data = cgss2021)

lm_local2test <- lm(protest_total ~ eva_loacl_z + age + female + edu + socailLevel_now  + lag(imcomePer)  + party + race + rural + health + marriage,data = cgss2021)

lm_cen2test <- lm(protest_total ~ eva_central_z + age + female + edu + socailLevel_now + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_local_med2test <- lm(protest_total ~ eva_loacl_z  + age + female + edu + socailLevel_now + mediaNew + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_cen_med2test <- lm(protest_total ~ eva_central_z  + age + female + edu + socailLevel_now + mediaNew + lag(imcomePer)  + party + race + rural + health + marriage,data = cgss2021)
...
:::{.callout-tip}
1. 地方政府：
  - 垃圾分类：直接效应显著，对地方政府信任程度越高，越容易参与垃圾分类；总效应要看中介变量，互联网使用(internetUse)无显著性，但新媒体使用(mediaNew)为的总效应正面影响;
  - 环境抗争：如果用新媒体使用(mediaNew)为中介变量，总效应只达到90%的显著性。但中介效应分析不显著，且为负面。
2. 中央政府：
  - 垃圾分类：直接效应不显著
...
```{r}
lm_ac_sys <- lm(action ~ trust_system + age + female + edu + socailLevel_now + internetUse + lag(imcomePer) + party + race + rural + health + marriage,data = cgss2021)

lm_total_sys <- lm(protest_total ~ trust_system + age + female + edu + socailLevel_now + internetUse + lag(imcomePer)  + party + race + rural + health + marriage,data = cgss2021)
...
modelsummary(mlist_sys2behavior,
             stars = TRUE,
             coef_rename = cm,
             output = "../outputs/results_sys2behavior.html")
...
:::{.callout-tip}
社会经济地位:socailLevel_now
价值信念规范:谁对环境负责？
政治参与、政府信任：政治参与最好是用政治效能感作为代理变量？还是说直接用政治参与(如是否参与过投票这些)，政府信任主要是对中央政府和地方政府的信任来衡量；
政治系统信任：目前是用对媒体的信任和对
:::
