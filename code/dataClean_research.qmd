...
df_resultMed_local2ac <- tibble(
  ...
  map(result_local2ac, function(Group) {
  map2_df(Group, ls_med, function(Result, Mediator) {
    tibble(
      mediator = Mediator,
      effect = "aceB",
      estimate = Result$d.avg,
      p.value = Result$d.avg.p,
      conf.low = Result$d.avg.ci[1],
      conf.high = Result$d.avg.ci[2],
      prop = scales::percent(Result$n.avg)
    ) %>%
      bind_rows(
        tibble(
          mediator = Mediator,
          effect = "ade",
          estimate = Result$z.avg,
          p.value = Result$z.avg.p,
          conf.low = Result$z.avg.ci[1],
          conf.high = Result$z.avg.ci[2],
          prop = ""
        )
      ) %>%
      bind_rows(
        tibble(
          mediator = Mediator,
          effect = "total",
          estimate = Result$tau.coef,
          p.value = Result$tau.p,
          conf.low = Result$tau.ci[1],
          conf.high = Result$tau.ci[2],
          prop = ""
        )
      )
  })
